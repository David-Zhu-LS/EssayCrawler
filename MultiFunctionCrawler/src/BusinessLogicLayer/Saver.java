 ...
 * EssaySaver(): Save the Essays we just crawled.
 * save(): Save the results generated by other functions.
 * It works following steps:
 * 1) If the lastOp is NewUrlCrawl or KeepUrlCrawl, save all the urls in the urlsHaveBeenFound.
 * 2) Otherwise, save all the things in the outputArea to a file named by the lastOp and lastUrl.
 * The default save path is ./saved.
 * The default filename is : modifiedUrl + lastOp.toString() + mode + ".txt";
 */
public class Saver {
    ...
    public static void essaySaver() {
        ...
        String path = defaultPath + "/Essay.txt";
        try {
            File fpath = new File(defaultPath), fout = new File(path);
